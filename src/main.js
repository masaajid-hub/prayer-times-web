...
async function handleAddressSearch() {
  ...
  try {
    ...
    if (coordinates) {
      ...
      showError(
        "Address not found. Please try a different location or enter coordinates manually."
      );
    }
  } catch (error) {
    showError(
      `Geocoding failed: ${error.message}. Please try entering coordinates manually.`
    );
  } finally {
...
async function detectTimezone(latitude, longitude) {
  try {
    ...
    const timezoneMap = [
      ...
      {
        bounds: { north: 23.4, south: 5.6, east: 109.5, west: 92.3 },
        tz: "Asia/Bangkok",
      }, // Thailand
      ...
      // South Asia
      {
        bounds: { north: 37, south: 5, east: 97, west: 60 },
        tz: "Asia/Karachi",
      }, // Pakistan
      {
        bounds: { north: 35.7, south: 6.4, east: 97.25, west: 68.1 },
        tz: "Asia/Kolkata",
      }, // India
      {
        bounds: { north: 26.8, south: 20.7, east: 92.7, west: 88 },
        tz: "Asia/Dhaka",
      }, // Bangladesh

      // North America
      {
        bounds: { north: 49.4, south: 24.4, east: -66.9, west: -125 },
        tz: "America/New_York",
      }, // Eastern US
      {
        bounds: { north: 49.4, south: 24.4, east: -87, west: -125 },
        tz: "America/Chicago",
      }, // Central US
      {
        bounds: { north: 49.4, south: 24.4, east: -104, west: -125 },
        tz: "America/Denver",
      }, // Mountain US
      {
        bounds: { north: 49.4, south: 24.4, east: -125, west: -180 },
      ...
      {
        bounds: { north: 51.1, south: 41.3, east: 9.6, west: -4.8 },
        tz: "Europe/Paris",
      }, // France
    ];
...
function displayWeekView(weekDays) {
  weekCalendar.innerHTML = weekDays
    .map(
      (day) => `
...
async function handleCalculate() {
  try {
    ...
    currentConfig = {
      ...currentCalcConfig,
      location: { latitude, longitude, date, timezone },
    };
...
function handleDownloadCsv() {
  if (!currentResults || !currentConfig) {
    showError(
      "No prayer times to download. Please calculate prayer times first."
    );
    return;
  }
...
function generateWeekText() {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (!times) {
        console.error(
          `Week export - Day ${i}: No times returned for date:`,
          date
        );
...
function generateMonthText() {
  try {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (!times) {
        console.error(
          `Month export - Day ${day}: No times returned for date:`,
          dayDate
        );
        continue; // Skip this day if no times returned
      }
  ...
  try {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      if (!firstTimes || !lastTimes) {
        console.error(`Year export - Missing times for ${monthName}:`, {
          firstTimes,
          lastTimes,
        });
...
function generateWeekCSV() {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (!times) {
        console.error(
          `Week CSV export - Day ${i}: No times returned for date:`,
          date
        );
        continue; // Skip this day if no times returned
      }
...
function generateMonthCSV() {
  try {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (!times) {
        console.error(
          `Month CSV export - Day ${day}: No times returned for date:`,
          dayDate
        );
        continue; // Skip this day if no times returned
      }
...
function generateYearCSV() {
  try {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      console.log(
        `Year CSV export - ${monthName} dates:`,
        firstDate,
        "to",
        lastDate
      );
      ...
      if (!firstTimes || !lastTimes) {
        console.error(`Year CSV export - Missing times for ${monthName}:`, {
          firstTimes,
          lastTimes,
        });
        continue; // Skip this month if calculation failed
...
function generateFilename(tabType) {
  ...
  const locationName = getLocationDisplayName(currentConfig.location).replace(
    /[^a-zA-Z0-9]/g,
    "_"
  );
...
function generateFullYearCsv() {
  try {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      console.log(
        `Full year CSV - Processing ${new Date(year, month - 1).toLocaleDateString("en-US", { month: "long" })}: ${daysInMonth} days`
      );
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (!times) {
          console.error(
            `Full year CSV - Missing times for ${dayDate.toDateString()}`
          );
